...
def estimate_complexity(code):
        ...
        if isinstance(node, (ast.For, ast.While)):
            ...
            for subnode in ast.walk(node):
                if isinstance(subnode, ast.Call) and isinstance(subnode.func, ast.Name):
                    if subnode.func.id == node.name:  # recursion detected
                        recursive_funcs.append(node.name)

    if recursive_funcs:
        time_complexity = "O(2^n)"  # naive assumption for recursion
    elif loop_count == 1:
        time_complexity = "O(n)"
    elif loop_count == 2:
    ...
    # Space heuristic (e.g., large list/range or nested structures)
